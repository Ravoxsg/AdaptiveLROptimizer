\documentclass{article}
  ...
  For the second-order method, we use the finite-differences approximation previously described. Here as well, the training set is considered as one large batch. We have set $\epsilon=10^{-5}$, so we now just need to choose an initial value $\eta_{ini}$. A value of $\eta_{ini}=10^{-2}$ gave the best convergence.
  ...
  From these first results, it seems that both the first-order and second-order methods are more efficient than a plain gradient descent. Indeed they reach lower training and test losses, after a comparable time of training, if not faster. However, in this case the second-order method presents some sudden and extreme amplitude variations, that we cannot really explain. The previous graphs omit such a spike at iteration 175, as including it would set the other curves to be flat lines. Instead of this spike, both loss values are 0. Such spikes were only seen in this Boston Housing dataset. 
  
  Note that while the first-order method steadily increases its learning rate, the second-order learning rate oscillates a lot. It is interesting to note that when training and test losses spike at 125 iterations, the second-order method reduces its learning rate value soon after, to stabilize training.
  ...
  The choice of $\epsilon$ was $10^{-5}$ and we used meta-learning rate $\alpha=10^{-6}$.
  \begin{figure}[!h]
